    ...
    try {
      ...
      if(req.status === 200 || req.status === 201){
        ...
        data.map((item,index) => {
          ...
          let theDistance = Math.floor(calculateDistance(targetLocation) / 1000)
  ...
  return (
    <ActionSheet ref={locationRef}>
      <View style={[t.bgwhite,t.wp100,t.px20,t.py20,t.brtl10,t.brtr10]}>
        <TouchableOpacity onPress={() => locationRef.current?.hide()} style={[t.msAuto]}>
        ...
        <ScrollView style={[t.mt20]} showsVerticalScrollIndicator={false}>
          {!loading ? studio.map((item,index) => {
            return (
              <LocationItem data={item} key={index} boxStyle={[t.mt10]} onSelectLocation={(selectedStudio)=>{setLocation(selectedStudio);locationRef.current?.hide()}}/>
            )
