...
function ContractCard({navigation, ...props}) {
  ...
  // {uri:data ? 'https://login.yogafitindonesiaonline.com/api/storage/qrcode/'+data.referal_code+'.png': 'xxx'}
  return (
    <Pressable
      onPress={() => {
        onPress(data);
      ...
      <View style={[t.relative]}>
        <AutoHeightImage source={img.card} style={[]} width={screenWidth} />
        <View style={[t.absolute, t.top20, t.right20, t.bgwhite, t.p5, t.br2]}>
          <Image
            source={img.logohorizontal}
            style={[t.w80, t.h17, {objectFit: 'contain'}]}
          />
        </View>
        <View style={[t.absolute, t.wp100, t.px20, t.bottom20]}>
          <Text style={[t['p14-600'], t.cwhite]}>{data && data.name}</Text>
          <Text style={[t['p12-600'], t.cwhite, t.mt10]}>
            {data && data.packages_name}
          </Text>
          <View style={[t.fRow, t.faCenter, t.fjBetween, t.mt10]}>
            <View style={[t.fRow, t.faCenter]}>
              ...
              <View style={[t.fRow, t.faCenter]}>
                ...
                <Text style={[t['p10-600'], t.cwhite]}>
                  {data && Helper.dateFormatId(data.end_date)}
                </Text>
              </View>
            </View>
            <Text style={[t['p12-400'], t.cwhite]}>
              {data && data.accesss == 'All'
                ? 'All Club Access'
                : 'Single Club Access'}
            </Text>
          </View>
          <Text style={[t['p12-600'], t.cwhite, t.mt15]}>
            {data && data.referal_code}
          </Text>
        </View>
      </View>
    </Pressable>
