...
import {
  View, ScrollView, StatusBar, Text, ActivityIndicator
} from 'react-native';
      ...
      <StatusBar translucent backgroundColor="transparent" barStyle="light-content" />
        ...
        <View style={[t.mt30,t.px20]}>
          <View style={[t.btw1,t.bsolid,t.bgreye]}>
            {!loading && faq.length > 0 ? faq.map((item,index) => {
              return (
                <Accordion title={item.title} content={item.faq} key={index}/>
              )
            }) : loading ? (<View style={[t.py50]}><ActivityIndicator size="large" color="#FE9805" /></View>) : <Text style={[t['p14-500'],t.cblack,t.tCenter,t.py50]}>No FAQ</Text>}
          </View>
        </View>
