...
const Location = ({navigation}) => {
  ...
  const calculateDistance = targetLocation => {
    if (location.latitude && location.longitude) {
      const calculatedDistance = getDistance(location, targetLocation);
    ...
    try {
      ...
      if (req.status === 200 || req.status === 201) {
        ...
        data.map(item => {
          ...
          item.distance =
            Math.floor(calculateDistance(targetLocation) / 1000) + ' km';
      ...
      <View style={[t.mt20, t.px20]}>
        ...
        {!loading ? (
          studio.map((item, index) => {
            return <LocationItem data={item} key={index} boxStyle={[t.mt10]} />;
          })
        ) : (
          <View style={[t.py50]}>
