  ...
  const getContract = async () => {
    ...
    try {
      ...
      if(req.status === 200 || req.status === 201){
        ...
        console.error("Error get packages")
      }
      setloading(false)
    } catch (error) {
      console.error(error)
  ...
  return (
    <ScrollView style={[t.bgwhite]}>
      ...
      <View style={[t.pt20, t.px20,t.bggreye,t.pb20]}>
        {!loading && contract ?
          ( <ContractCard data={contract} boxStyle={[t.mt10]} onPress={(data)=>{setqrcode({uri:data ? 'https://login.yogafitidonline.com/api/storage/qrcode/'+data.referal_code+'.png': 'xxx'});qrRef.current?.show()}}/>)
      ...
      <View style={[t.mt20, t.px20]}>
        {!loading && packages.length > 0 ? packages.map((item,index) => {
          return (
            <PackagesCard data={item} key={index} boxStyle={[t.mt10,t.bbw1,t.bsolid,t.bgreyd,t.pb10]}/>
          )
        }) : loading ? (<View style={[t.py50]}><ActivityIndicator size="large" color="#FE9805" /></View>) : <Text style={[t['p14-500'],t.cblack,t.tCenter,t.py50]}>No Available Contract</Text>}
      </View>
